...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                PmGenerator.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(PmGenerator.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + PmGenerator.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == PmGenerator.EVIDENCE_BEFORE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_PMGEN_BEFORE"+ "." + PmGenerator.IMAGE_EXTENSION);}
        else if (type == PmGenerator.EVIDENCE_PROCESS) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_PMGEN_PROCESS"+ "." + PmGenerator.IMAGE_EXTENSION);}
        else if (type == PmGenerator.EVIDENCE_AFTER) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_PMGEN_AFTER"+ "." + PmGenerator.IMAGE_EXTENSION);
        }
